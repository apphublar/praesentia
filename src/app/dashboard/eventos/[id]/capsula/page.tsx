...
import { AdminCapsulaExperience } from "@/components/app/admin/admin-capsula-experience";
...
export default async function AdminCapsulaPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = await params;
  const loginNext = `/dashboard/eventos/${id}/capsula`;
  ...
  const [media, guestRsvps] = await Promise.all([
  ...
  const confirmedGuestCount = guestRsvps.filter((row) => row.rsvpStatus === "confirmed").length;

  return <AdminCapsulaExperience event={event} media={media} confirmedGuestCount={confirmedGuestCount} />;
}
